...
export type PaymentApprovedItem = {
  tid: string;
};
...
export async function getPaymentApprovedInfo(
  data: UserProfile,
): Promise<PaymentApprovedItem[] | null> {
  try {
    const response = await api.get<PaymentApprovedItem[]>(
      `/payment/approved/${data.id}/`,
    );
    if (response.status === 200) {
      return response.data;
    }
...
export async function getPaymentInfo(
  data: PaymentApprovedItem,
): Promise<PaymentDetail | null> {
  try {
    const response = await api.get<PaymentDetail>(
      `/payment/detail/${data.tid}/`,
    );
    if (response.status === 200) {
      return response.data;
    }
